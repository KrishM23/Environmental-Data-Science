...
Recreate Terrain's homepage carbon tracker: verified UCLA greenhouse gas totals (Scopes 1+2+3), a trend from the 2019 baseline, and UC's **90% reduction by 2045** commitment.
...
**Time:** ~1.5 hr · Compare with [live tracker](../index.html#tracker) · [Explainer](../explainer.html?id=ucla-emissions)
...
- **Tip:** Open [Terrain's tracker](../index.html#tracker) side-by-side and compare lines.
...
**Goal:** Import the same annual totals Terrain uses on the homepage.
...
load_emissions <- function() {
  ...
  read_csv(paste0(GITHUB_RAW, "/ucla_emissions.csv"), show_col_types = FALSE)
}
...
**Goal:** Quantify recent direction with `lm()` — same idea as the blue trend on Terrain.
...
**Goal:** Layer three stories on one plot: **reported** (dots), **trend** (blue), **commitment pace** (dashed orange).

**You should see:** Chart matching the structure of [Terrain's live tracker](../index.html#tracker).
...
**Next:** [Live tracker](../index.html#tracker) · [Programs](../programs.html)
